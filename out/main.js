...
async function compressImage(filePath, format, quality) {
  const ext = path.extname(filePath).toLowerCase();
  if (supportedFormats.includes(ext.slice(1))) {
    const outputFilePath = `${filePath}-compressed.${format}`;
    await (0, import_sharp.default)(filePath).rotate().toFormat(format, { quality: parseInt(quality) }).withMetadata().toFile(outputFilePath);
    console.log(`Compressed: ${outputFilePath}`);
  }
}
async function compressImagesInDirectory(folderPath, format, quality) {
